...
latex_train= load('dataset/featureMat_Latex_train_bioLBP.mat')
%%
latex_matrix = zeros();
latex_matrix = latex_train.featureMat_Latex_train_bioLBP;
%% Load liv training data

liv_train=load('dataset/featureMat_liv_train_bioLBP.mat')
...
latex_test= load('dataset/featureMat_Latex_test_bioLBP.mat');
%%
latex_test_matrix = zeros();
latex_test_matrix = latex_test.featureMat_Latex_test_bioLBP;
%% Load liv testing data

liv_test=load('dataset/featureMat_liv_test_bioLBP.mat');
...
gela_train= load('dataset/featureMat_Gelatine_train_bioLBP.mat')
...
gela_test= load('dataset/featureMat_Gelatine_test_bioLBP.mat');
%%
...
%%
data_test1=[latex_test_matrix;liv_test_matrix;gel_test_matrix];
data_test_label1=[latex_test_label;liv_test_label;gel_test_label];

%% Start testing
[label1, Posterior1, cost1] = predict(model2, data_test1);

%% Calculate loss and resubstitution error

L1 = loss(model2,data_test1,data_test_label1);
resubstitutionLoss1 = resubLoss(model2);

%% After changing priors
model1.Prior = [0.4 0.6];
[label_afterpriorchange, Posterior_afterpriorchange, cost_afterpriorchange] = predict(model1, data_test);
%% loss and resubstitution error after changing priors
L_priors = loss(model1,data_test,data_test_label);
resubstitutionLoss_priors = resubLoss(model1);
